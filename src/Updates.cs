...
namespace Digitone
{
    ...
    public sealed partial class PlayerApp
    {
        ...
        {
            string version=release.Version.TrimStart('v','V');var result=MessageBox.Show(Window,"Digitone "+version+" is here!\n\nReady to check out the new update? Digitone will download, verify, and install it for you while keeping your Data and DigiMusic folders safe.\n\nA direct YouTube option is also being tested right now, so keep an eye out for that!\n\nIf you find an issue, know a possible fix, or have an idea you would love to see, please shoot me a DM! No idea is a bad one. If something feels off to you, there is a good chance someone else feels it too, haha.\n\nYou can check out everything new in the Changelog menu.\n\nInstall Digitone "+version+" now?","Digitone update!",MessageBoxButton.YesNo,MessageBoxImage.Information);if(result!=MessageBoxResult.Yes)return;installingUpdate=true;try{var plan=await Task.Run(delegate{return DigitoneUpdates.Prepare(release,delegate(string message){Window.Dispatcher.BeginInvoke(new Action(delegate{Status(message);}));});});string planPath=Path.Combine(plan.StagingRoot,"update-plan.json");File.WriteAllText(planPath,new JavaScriptSerializer().Serialize(plan));string helper=Path.Combine(plan.PackageRoot,"Digitone.exe");Process.Start(new ProcessStartInfo(helper,"--apply-update \""+planPath+"\""){UseShellExecute=true,WorkingDirectory=plan.PackageRoot});Application.Current.Shutdown();}catch(Exception error){installingUpdate=false;MessageBox.Show(Window,"Digitone could not prepare this update. Nothing was changed.\n\n"+error.Message,"Update stopped",MessageBoxButton.OK,MessageBoxImage.Warning);}
        }
