...
namespace Digitone
{
    public sealed partial class PlayerApp
    {
        ...
        internal Window CreateSettings()
        {
            ...
            var audioButton = new Button { Content = "Audio", Margin = new Thickness(0,0,0,8), HorizontalContentAlignment = HorizontalAlignment.Left };
            ...
            var generalButton = new Button { Content = "General", Margin = new Thickness(0,0,0,8), HorizontalContentAlignment = HorizontalAlignment.Left };
            nav.Children.Add(appearanceButton);nav.Children.Add(audioButton);nav.Children.Add(libraryButton);nav.Children.Add(generalButton);
            ...
            StackPanel appearance=new StackPanel(),audio=new StackPanel(),library=new StackPanel(),general=new StackPanel();
            appearance.Children.Add(SettingsText("Appearance",28));appearance.Children.Add(SettingsText("Choose the color of your signal.",13,true));audio.Children.Add(SettingsText("Audio",28));audio.Children.Add(SettingsText("Choose where and how Digitone plays your music.",13,true));library.Children.Add(SettingsText("Music folders",28));library.Children.Add(SettingsText("Choose where your collection lives.",13,true));general.Children.Add(SettingsText("General",28));general.Children.Add(SettingsText("Controls, download tools, updates, and information.",13,true));
            var mediaKeys = new CheckBox { Content = "Global media keys", IsChecked = Data.GlobalMediaKeys != false, Margin = new Thickness(0,0,0,8) };
            var mediaOptions = new StackPanel(); mediaOptions.Children.Add(mediaKeys); mediaOptions.Children.Add(SettingsText("Play, pause and skip while Digitone is in the background. Turn off to let another player use these keys. System volume keys stay with Windows.",12,true)); general.Children.Add(SettingsCard(mediaOptions));
            ...
            var outputPanel=new StackPanel{Name="AudioOutputSettingsSection"};outputPanel.Children.Add(SettingsText("Audio output",16));outputPanel.Children.Add(SettingsText("Choose where Digitone sends music. Windows default follows your current system output and is safest for unplugged devices.",12,true));
            var outputs=AudioPlayback.Outputs();var outputChoice=new ComboBox{Name="AudioOutputSelector",ItemsSource=outputs,Margin=new Thickness(0,8,0,0),MinWidth=360,HorizontalAlignment=HorizontalAlignment.Left};var savedOutput=outputs.FirstOrDefault(o=>String.Equals(o.Id,Data.AudioOutputId,StringComparison.OrdinalIgnoreCase));outputChoice.SelectedItem=savedOutput??outputs.First();outputChoice.SelectionChanged+=delegate{var selected=outputChoice.SelectedItem as AudioOutputChoice;if(selected!=null)ChangeAudioOutput(selected.Id);};outputPanel.Children.Add(outputChoice);audio.Children.Add(SettingsCard(outputPanel));
            ...
            var sectionButtons=new[]{appearanceButton,audioButton,libraryButton,generalButton};Action<StackPanel,Button> show=delegate(StackPanel panel,Button selected){scroll.Content=panel;foreach(var button in sectionButtons)button.SetResourceReference(Control.BackgroundProperty,button==selected?"B354233":"B1C211D");scroll.ScrollToTop();Motion.Enter(panel,18,0);};
            appearanceButton.Click+=delegate{show(appearance,appearanceButton);};audioButton.Click+=delegate{show(audio,audioButton);};libraryButton.Click+=delegate{show(library,libraryButton);};generalButton.Click+=delegate{show(general,generalButton);};
            ...
            previewScroll.PreviewMouseWheel+=delegate(object sender,System.Windows.Input.MouseWheelEventArgs e){if((System.Windows.Input.Keyboard.Modifiers&System.Windows.Input.ModifierKeys.Shift)!=0)previewScroll.ScrollToHorizontalOffset(previewScroll.HorizontalOffset-e.Delta);else scroll.ScrollToVerticalOffset(scroll.VerticalOffset-e.Delta);e.Handled=true;};
            ...
            dialog.Tag = libraryButton; refresh(); refreshThemes(); show(appearance,appearanceButton); return dialog;
        }
