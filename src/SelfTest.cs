                ...
                player.Window.Loaded += async delegate
                {
                    try
                    {
                        ...
                        if (AudioDownloader.ToolsReady)
                        {
                            ...
                            var generalNav=Elements(settingsWindow).OfType<Button>().Single(b=>Convert.ToString(b.Content)=="General");generalNav.RaiseEvent(new RoutedEventArgs(Button.ClickEvent));await Task.Delay(60);var changeBullets=AppInfo.Changelog.Split(new[]{'\r','\n'},StringSplitOptions.RemoveEmptyEntries).Where(line=>line.StartsWith("•")).ToArray();Check(Elements(settingsWindow).OfType<TextBlock>().Any(t=>t.Name=="VersionLabel"&&t.Text=="Version "+AppInfo.Version)&&Elements(settingsWindow).OfType<Button>().Any(b=>b.Name=="ChangelogButton")&&Elements(settingsWindow).OfType<CheckBox>().Any(c=>Convert.ToString(c.Content)=="Global media keys")&&Elements(settingsWindow).OfType<CheckBox>().Any(c=>Convert.ToString(c.Content).StartsWith("Automatically find"))&&AppInfo.ChangelogVersions.Length==AppInfo.Changelogs.Length&&AppInfo.ChangelogVersions.Length>=2&&AppInfo.Release1Changelog.Contains("PLAYBACK AND SOUND")&&changeBullets.Length==changeBullets.Distinct().Count(),"General tab contains controls, updates, tools, credits, versioning, and retained changelogs");
